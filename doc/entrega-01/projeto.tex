\documentclass[a4paper, 12pt]{article}

\usepackage[brazilian]{babel}
\usepackage{ae,aecompl}
\usepackage[utf8]{inputenc}

\usepackage{fullpage}  
\usepackage{indentfirst}      % Indenta o primeiro paragrafo das sections
\usepackage{graphicx}         % Utilizado para inserir imagens
\usepackage{hyperref}         % Cria link entre o sumario e as sections
\usepackage{fancyhdr}         % Permite alterar o header e footer
\usepackage{mdwlist}          % Permite criar listas com menos espaçamento
\usepackage[includefoot]{geometry}

% Seta o estilo padrao
\pagestyle{fancy}

% Configura links
\hypersetup{colorlinks=true,linkcolor=black,citecolor=black}

% Seta espacamento
\addtolength{\parskip}{2mm}
\addtolength{\footskip}{15mm}

% Coloca no p√© a pagina atual
\begin{document}

% Inclua as variáveis
\input{variaveis}

% Inclua header
\input{header}

% Inluia a capa
\newpage
\input{capa.tex}

% Seta o footer das paginas iniciais para numeros Romanos
\cfoot{\thepage}
\setcounter{page}{1}
\pagenumbering{Roman}

% Sum√°rio
\newpage
\tableofcontents

% Lista de figuras
\newpage
\listoffigures

% Lista de tabelas
\newpage
\listoftables

% Seta o footer das paginas para numeros convencionais
\clearpage
\setcounter{page}{1}
\pagenumbering{arabic}

% PARA INSERIR NOVAS PAGINAS BASTA ADICIONAR AQUI:
% Conteúdo
\include{capit/intro}
\include{capit/casos_de_uso}
\include{capit/modelo_de_dados}
\include{capit/mockups}
\include{capit/webservices}
\include{capit/pesquisa_libs}
\include{capit/pesquisa_ocr}

\clearpage

% Glossário
%\include{glossario}

\end{document}
